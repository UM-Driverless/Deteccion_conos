...
class VisualizeActuators(VisualizeInterface):
    ...
    def __init__(self, max_data_to_store=10000):
        ...
        self.upgear_historial = []
        self.downgear_historial = []

        self.gear_historial = []
        self.speed_historial = []
    ...
    def visualize(self, can_data, agent_data, print_can_data=True, print_agent_actions=True, real_time=False):
        [in_speed, in_throttle, in_steer, in_brake, in_clutch, gear] = can_data
        [throttle, brake, steer, clutch, upgear, downgear] = agent_data
        ...
        self.upgear_historial.append(int(upgear))
        self.downgear_historial.append(int(downgear))

        self.gear_historial.append(gear)
        self.speed_historial.append(in_speed)
    ...
    def print_can_data(self):
        ...
        ax.plot(lin_space, self.speed_historial)
    ...
    def print_agent_data(self):
        ...
        ax = fig.add_subplot(3, 3, 5)
        ax.plot(lin_space, self.upgear_historial)
        ax.set(xlabel='steps', ylabel='force', title='upgear')

        ax = fig.add_subplot(3, 3, 6)
        ax.plot(lin_space, self.downgear_historial)
        ax.set(xlabel='steps', ylabel='force', title='downgear')
