...
class Agent():
    ...
    def get_target(self, cones, car_state, agent_act):
        ...
        if (len(yellows)>1 and len(blues)>1):
            ...
            extra = (middleCone0[0] * (car_state['speed'] + 0.01)) / (ifps[0]+0.001) + (agent_act['acc']/(ifps[0]+0.001))#(middleCone0[0] * (ifps[0])) / (car_state['speed'] + 0.01)  #distanci recorrids en un fotograma
            print(f'BBBBBBBBBBBBBBBBBBBB {extra}')
            A = np.array(
            ...
            if maxSpeed > car_state['speed'] > (maxSpeed / 2):
                agent_act['acc'] = 0.2
            agent_act['steer'] = 1 if len(yellows)>len(blues) else lastAngle[0]/60 if (len(yellows)==0 and len(yellows)==len(blues)) else 0 if len(yellows)==len(blues) else -1
