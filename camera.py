...
class Camera(ABC):
    ...
    def create(mode: int, cam_queue):
        ...
        """
        if mode == 0:
            return ImageFileCamera(cam_queue, 'test_media/cones_image.png')
        elif mode == 1:
            return VideoFileCamera(cam_queue, 'test_media/video.mp4')
        elif mode == 2:
            return WebcamCamera(cam_queue, 0)
        elif mode == 3:
            return ZedOpenCVCamera(cam_queue)
        elif mode == 4:
            return SimulatorCamera(cam_queue)
        elif mode == 5:
            return SimulatorWatch(cam_queue)
        else:
            raise ValueError("Unknown camera type")

class CameraFactory:
    @staticmethod
    def get_camera(camera_type: int, cam_queue):
        if camera_type == 0:
            # From Image file
            return ImageFileCamera(cam_queue)
        elif camera_type == 1:
            # From Video file
            return VideoFileCamera(cam_queue)
        else:
            raise ValueError("Unknown camera type")
...
class VideoFileCamera(Camera):
    def __init__(self, cam_queue, VID_PATH):
        super().__init__(cam_queue)
...
class WebcamCamera(Camera):
    def __init__(self, cam_queue, cam_index):
        super().__init__(cam_queue)
...
class SimulatorCamera(Camera):
    """
    Camera class for reading images from a simulator, and controlling the car from the resulting values. TODO MIXING BEHAVIOR, THINK.
